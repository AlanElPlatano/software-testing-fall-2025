...
from tdd.string_calculator.v8.string_calc import add_strings as add
...
class TestStringCalculator(unittest.TestCase):
    ...
    # Tests for ignoring numbers bigger than 1000 (v8)
    def test_number_bigger_than_1000_ignored(self):
        """Test that numbers bigger than 1000 are ignored"""
        result = add("1001,2")
        self.assertEqual(result, 2)

    def test_number_1000_included(self):
        """Test that number 1000 is included"""
        result = add("1000,2")
        self.assertEqual(result, 1002)

    def test_multiple_numbers_above_1000_ignored(self):
        """Test that multiple numbers above 1000 are ignored"""
        result = add("2,1001,1002,3")
        self.assertEqual(result, 5)

    def test_all_numbers_above_1000(self):
        """Test that all numbers above 1000 return 0"""
        result = add("1001,1002,1003")
        self.assertEqual(result, 0)

    def test_mixed_numbers_with_custom_delimiter(self):
        """Test mixed numbers with custom delimiter, ignoring >1000"""
        result = add("//;\n1000;1001;2")
        self.assertEqual(result, 1002)

    def test_negative_above_1000_still_negative(self):
        """Test that negative numbers above 1000 are still caught"""
        with self.assertRaises(ValueError) as context:
            add("1001,-1002")
        self.assertIn("Negative number(s) not allowed: -1002", str(context.exception))

    def test_negative_and_above_1000(self):
        """Test combination of negative and above 1000 numbers"""
        with self.assertRaises(ValueError) as context:
            add("1001,-1002,3")
        self.assertIn("Negative number(s) not allowed: -1002", str(context.exception))
